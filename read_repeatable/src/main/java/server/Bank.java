...
public class Bank implements Runnable {
    ...
    public Bank (String serverName, String MQTTTopic){
...
//        blockingConnection.publish(path+"_BankService", ans.toString().getBytes(), QoS.EXACTLY_ONCE, false);
    }

    void subscribeMQTT() throws Exception {
//        Topic[] topics = { new Topic("BankService", QoS.EXACTLY_ONCE) };
        ...
        zk.addWatch(Path, new Watcher() {
            ...
            public void process(WatchedEvent watchedEvent) {
                if (watchedEvent.getType().equals(Event.EventType.NodeDataChanged)){
                    try {
                        String selectTxLogSQL = String.format("SELECT Transaction,Branch,Money FROM Bank_Logs WHERE Transaction = \"%s\"",txIndex);
                        ResultSet resultSet = stmt.executeQuery(selectTxLogSQL);
                        System.out.println("有到這裡 : selectTxLogSQL");
                        while (resultSet.next()){
                            System.out.println("AAAAAAA");
                            int item = resultSet.getInt("Branch");
                            int num = resultSet.getInt("Money");
                            String updateDatabaseSQL = String.format("UPDATE Bank_Record SET Balance = Balance + %d WHERE Branch = %d",num,item);
                            stmt.execute(updateDatabaseSQL);
                            String readSQL = String.format("Select %s From %s where Branch = %s",column,table,row);
                            try {
            ...
            public void processResult(int i, String s, Object o) {
                switch (KeeperException.Code.get(i)) {
                    ...
                    case OK:
                        ...
                        String txLogSQL = String.format("INSERT INTO Bank_Logs(Transaction,Branch,Money) VALUES (\"%s\",%d,%d)",txIndex,row,data); // DB的item從0開始到99，row看需不需要-1
                        try {
        ...
        zk.addWatch(Path, new Watcher() {
            ...
            public void processResult(int i, String s, Object o) {
                switch (KeeperException.Code.get(i)) {
                    ...
                    case OK:
                        try {
                            String txLogSQL = String.format("INSERT INTO Bank_Logs(Transaction,Branch,Money) VALUES (\"%s\",%d,0)",txIndex,row); // DB的item從0開始到99，row看需不需要-1
                            stmt.execute(txLogSQL);
                            twoPhaseCommit = "Prepare" ;
                        } catch (SQLException e) {
                            throw new RuntimeException(e);
