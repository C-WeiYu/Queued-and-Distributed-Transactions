...
public class Teller implements Runnable {
    ...
    public Teller(String serverName,String MQTTTopic) {
...
//        blockingConnection.publish(path+"_TellerService", ans.toString().getBytes(), QoS.EXACTLY_ONCE, false);
    }

    void subscribeMQTT() throws Exception {
//        Topic[] topics = { new Topic("TellerService", QoS.EXACTLY_ONCE) };
    ...
    void fixMessageToSQL(String Message){  //Message= "Tx_0000000001,Table/Column/9,5"
        ...
        lockPath = "/Data/"+ serverName + "/" + table + "/" + column + "/" + row + "/Write";
//        System.out.println("Teller : lockPath = " + lockPath);
    ...
    void createNode(String Path){
        zk.create(Path, twoPhaseCommit.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL, new AsyncCallback.StringCallback() {
            @Override
            public void processResult(int i, String s, Object o, String s1) {
                System.out.println("Teller : " + s1);
                System.out.println("Teller : " + txIndex);
...
//                    addWatch(s1,items[3],searchSQL(items));
        ...
        zk.addWatch(Path, new Watcher() {
            ...
            public void process(WatchedEvent watchedEvent) {
                if (watchedEvent.getType().equals(Event.EventType.NodeDataChanged)){
                    try {
                        String selectTxLogSQL = String.format("SELECT Transaction,Teller,Money FROM Teller_Logs WHERE Transaction = \"%s\"",txIndex);
                        ResultSet resultSet = stmt.executeQuery(selectTxLogSQL);
                        while (resultSet.next()){
                            int item = resultSet.getInt("Teller");
                            int num = resultSet.getInt("Money");
                            String updateDatabaseSQL = String.format("UPDATE Teller_Record SET Balance = Balance + %d WHERE Teller = %d",num,item);
                            stmt.execute(updateDatabaseSQL);
                            String readSQL = String.format("Select %s From %s where Teller = %s",column,table,row);
                            try {
            ...
            public void processResult(int i, String s, Object o) {
                switch (KeeperException.Code.get(i)) {
                    ...
                    case OK:
                        String txLogSQL = String.format("INSERT INTO Teller_Logs(Transaction,Teller,Money) VALUES (\"%s\",%d,%d)",txIndex,row,data); // DB的item從0開始到99，row看需不需要-1
                        try {
    ...
    void addWatch_Read(String Path ,String txIndex){
        zk.addWatch(Path, new Watcher() {
            @Override
            public void process(WatchedEvent watchedEvent) {
                if (watchedEvent.getType().equals(Event.EventType.NodeDataChanged)){
            ...
            public void processResult(int i, String s, Object o) {
                switch (KeeperException.Code.get(i)) {
                    ...
                    case OK:
                        try {
                            String txLogSQL = String.format("INSERT INTO Teller_Logs(Transaction,Teller,Money) VALUES (\"%s\",%d,0)",txIndex,row); // DB的item從0開始到99，row看需不需要-1
                            stmt.execute(txLogSQL);
                            twoPhaseCommit = "Prepare" ;
                        } catch (SQLException e) {
                            throw new RuntimeException(e);
